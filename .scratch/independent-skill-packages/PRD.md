...
本文是待用户评审的完整实施方案。`ready-for-agent` 表示工单信息完整，不表示已授权本任务进入实现。本次仅发布规划；以下测试均为实施验收要求，未声称已通过。
...
不内置、fork、重写、自动安装 Matt；不修改 codex-manager 或第三方 installer；不新增通用安装器、依赖求解器、自动更新服务、后台 monitor；不改变五阶段业务职责、模型路由、文档权限或 Git/ledger 权威；不新增平台适配；不实现 version 1 runner 的自动迁移；不更改用户全局安装。本轮只做方案，不实现、不部署、不远程发布、不进入 Stage 3/4。
...
### 当前生产调用链证据（基线 78913735a91dcd4c3b343c0a097e2afa05981b36）
...
按 ask-matt 的多会话构建分支，本任务跨构建、运行时、五个阶段和兼容恢复，值得拆 Tickets。使用 to-tickets 的 expand–migrate–contract 例外：先提供可验证生成面，再迁移独立阶段，再去掉旧来源；每片都有可观察结果。用户此次授权完整方案先成稿，测试 seam 和 Tickets 粒度不再制造中间停顿，最终一并评审。
...
01–04 可在同一集成分支分批提交，过渡阶段生成目录不得成为第二个仓库可发现入口（临时构建仅在仓库外）；最终 05 将唯一发布面落到约定位置，删除旧源入口，保持所有五个用户安装路径。每片允许在仓库外验证待生成包；最终才宣称五个安装包整体交付。规划就绪：模块 owner、接口、失败分支、状态恢复、兼容决策与测试 seam 均已确定；实现仍需完成所有代码、测试与现场证据。
