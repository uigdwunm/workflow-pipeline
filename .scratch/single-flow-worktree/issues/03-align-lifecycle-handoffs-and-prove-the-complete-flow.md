...
- [x] A fresh standalone `1拷问 → 2方案 → 3实现 → 4归档` exercise uses exactly one Flow Worktree from Stage 2 through final cleanup.
- [x] A fresh standalone Stage-3 exercise creates exactly one Flow Worktree and hands it to Stage 4 for final cleanup.
- [x] Planning publication with an unrelated target advance continues without user input; a controlled real conflict preserves state and resumes only after user confirmation.
