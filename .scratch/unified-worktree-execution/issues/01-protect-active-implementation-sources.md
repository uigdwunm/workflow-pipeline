...
- [ ] A prepared implementation binds one committed source checkpoint and an
      exact non-empty artifact set containing repository-relative path, Git blob
      identity and SHA-256 digest.
- [ ] `supervision_protocol.py` owns one repository-scoped source-protection
...
- [ ] Source protection is created before worktree provisioning and remains
      active until every dependent implementation is archived or verifiably
      cancelled.
- [ ] Every planning-document lease/write entry point rejects a protected path
      before mutation. The only exception is exact Stage-4 convergence after no
      still-running dependent implementation remains.
- [ ] A marker is never implemented by editing source-document bytes after the
      checkpoint. Manual or external byte changes produce typed source drift and
      are never overwritten.
- [ ] A requested source change supports only a separately identified successor
      ...
      ACK state remains.
- [ ] One implementation record enforces
...
- [ ] CLI tests cover multiple implementations sharing one source, unrelated
