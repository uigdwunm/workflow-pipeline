...
- [ ] Candidate review and acceptance bind one exact implementation commit,
      source checkpoint and verification record; changed candidate identity
      invalidates both review and acceptance.
- [ ] The Stage-3 supervisor is the only actor that performs final integration
      and uses the verified primary checkout only for the bounded publication
      operation.
- [ ] Publication refuses a dirty checkout, wrong branch, in-progress Git
...
- [ ] A short target-branch publication slot serializes workflow publishers and
      an expected-HEAD CAS detects external ref movement immediately before the
      merge effect.
- [ ] An advanced target branch no longer fails merely because it differs from
...
- [ ] Merge failure or conflict aborts in the primary checkout and proves branch,
      HEAD, index, worktree and managed-document restoration before returning
      control to the original implementation worktree.
- [ ] Textual/structural repair and local semantic adaptation that preserve
