...
def skill_name(entry):
    match = re.search(r'^name:\s*([a-z][a-z0-9-]*)\s*$', entry.read_text(encoding='utf-8'), re.MULTILINE)
    if not match:
...
def package_identity(value, name=None):
    ...
    try:
        ...
        if manifest.get('stage') != STAGES.index(actual_name):
        ...
        for path in root.rglob('*'):
            ...
            if path.is_file() and '__pycache__' not in path.parts and path.name != 'package.json':
...
def preflight(request):
    ...
    if action == 'selected-capability':
        if not selected or any(not isinstance(n,str) or not re.fullmatch('[a-z][a-z0-9-]*',n) or n in STAGES for n in selected):
