...
Read [Workflow Control Protocol]({{resource:guided-implementation/references/workflow-control-protocol.md}}).
...
single fixed completion confirmation from `{{resource:problem-framing/references/templates.md}}` after the
...
frozen source host, and the delivery payload from `{{resource:problem-framing/references/templates.md}}`. Never search
