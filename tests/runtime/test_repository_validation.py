...
class RepositoryValidationTests(unittest.TestCase):
    ...
    def test_topic_dependency_wrapper_contracts_are_shipped(self) -> None:
        ...
        handoff = (REPOSITORY / "skills/design-discussion/references/shared/design-discussion/child-topic-protocol.md").read_text(encoding="utf-8")
        templates = (REPOSITORY / "skills/design-discussion/references/shared/design-discussion/templates.md").read_text(encoding="utf-8")
        split_gate = (REPOSITORY / "skills/design-discussion/references/shared/design-discussion/split-gate-contract.md").read_text(encoding="utf-8")
...
确认方式：回复 `确认`"""
    ...
    def test_stage_three_uses_executable_thread_settings_verification(self) -> None:
        ...
        if (REPOSITORY / "skills/guided-implementation/references/shared/guided-implementation/workflow-control-protocol.md").exists():
    ...
    def test_stage_four_uses_the_existing_topic_local_three_to_four_run(self) -> None:
        ...
        lifecycle = (
            REPOSITORY
            / "skills/design-discussion/references/shared/design-discussion/lifecycle-integration.md"
        ).read_text(encoding="utf-8")
        combined = closure + "\n" + lifecycle
    ...
    def test_closure_verifies_worktree_and_routes_unclear_representation(self) -> None:
        ...
        actions = (
            REPOSITORY / "skills/change-closure/references/change-closure/closure-actions.md"
        ).read_text(encoding="utf-8")
    ...
    def test_phase_one_always_owns_one_current_requirement_document(self) -> None:
        ...
        requirement_contract = (
            REPOSITORY
            / "skills/design-discussion/references/shared/design-discussion/requirement-document-contract.md"
        ).read_text(encoding="utf-8")
        solution = (
            REPOSITORY / "skills/solution-design/SKILL.md"
        ).read_text(encoding="utf-8")
        ...
        self.assertIn("one committed requirement document", solution_protocol)
        self.assertIn("The child neither supplements it from chat", solution)
        self.assertIn("before creating a Flow Worktree", solution)
        self.assertIn("pre-launch anomaly", solution)
        self.assertIn("before `start-worktree` or launch", solution_protocol)
        self.assertIn("An invalid claimed handoff remains an anomaly", solution_protocol)
    ...
    def test_confirmations_bind_context_not_magic_strings(self) -> None:
        confirmation = (
            REPOSITORY
            / "skills/design-discussion/references/shared/design-discussion/confirmation-contract.md"
        ).read_text(encoding="utf-8")
        for marker in (
    ...
    def test_child_topic_reference_reaches_the_real_codex_task_seam_in_order(self) -> None:
        reference = (
            REPOSITORY
            / "skills/design-discussion/references/shared/design-discussion/child-topic-protocol.md"
        ).read_text(encoding="utf-8")
        ordered = [
    ...
    def test_test_discovery_is_dynamic_and_sorted(self) -> None:
        temporary_directory, repository = self.make_repository()
        self.addCleanup(temporary_directory.cleanup)
        first = repository / "tests" / "test_zeta.py"
        second = repository / "tests" / "test_alpha.py"
        nested = repository / "tests" / "nested" / "test_nested.py"
    ...
    def test_documentation_classifier_matches_protocol_scope_without_absorbing_code(self) -> None:
        documentation = {
            "docs/skills/example.md", "skills/unrelated/README.md", "CONTRIBUTING.md", "CHANGELOG.md", "SECURITY.md", "architecture.md",
        ...
        implementation = {
            ...
            "skills/guided-implementation/SKILL.md", "skills/design-discussion/references/shared/contract.md",
            "src/stages/design-discussion/SKILL.md.in", "src/shared/references/contract.md",
    ...
    def test_dependency_check_reports_candidates_without_claiming_activation(self) -> None:
        temporary_directory = tempfile.TemporaryDirectory()
        self.addCleanup(temporary_directory.cleanup)
        ...
        completed = subprocess.run(
            [str(DEPENDENCY_CHECK), "--stage", "1", "--action", "route"],
            stdout=subprocess.PIPE,
            stderr=subprocess.PIPE,
            text=True,
        ...
        self.assertIn("filesystem-only", completed.stdout)
        self.assertFalse(json.loads(completed.stdout)["callable"])
        self.assertEqual(len(json.loads(completed.stdout)["skills"]["ask-matt"]), 2)
